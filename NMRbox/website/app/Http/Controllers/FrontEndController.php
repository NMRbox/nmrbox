...
class FrontEndController extends Controller
{
                ...
                if ($parse_token == true)
                {
                    ...
                    foreach ($user_classification as $key => $value) {
                        ...
                        if( Session::has('user_is_admin') === true ) {
                            echo "user in ";
                            exit();
                            Redirect::to('dashboard')->with('success', 'You have successfully logged in!');
                        } else {
                            echo "user out";
                            exit();
    ...
    public function downloadableVM(Request $request)
    {
        ...
        $person = $this->sessionPlayLoad( $person_id );
        $vmId = Input::get('vm_id');

        $downloadable_vm = new VMDownload();
        $isCheck = VMDownload::where('person_id', $person->id )->where('vm_id', $vmId)->get()->first();
